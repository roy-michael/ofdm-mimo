...
def main():
    ...
    rx_tx_list = [(2, 2), (2, 4)]
    ser_array = np.zeros((len(snr_db_range), len(rx_tx_list)))

    for rx_tx_idx, (rx, tx) in enumerate(rx_tx_list):
        ...
        for snr_db_idx, snr_db in enumerate(snr_db_range):
            ...
            s_detected = _ml_detect(s_hat.flatten(), constellation)

            # Count the errors and calculate SER
            # ser_stc_array[snr_db_idx] = np.sum(s_stc_int != s_stc_detected) / (num_symbols) # TODO: * 2 symbols per STC block ?
            ser_array[snr_db_idx, rx_tx_idx] = np.sum(s_int != s_detected) / num_symbols

    # --- Plotting ---
    plt.figure(figsize=(10, 6))

    styles = ['r-', 'b--']
    for mrc_idx, mrc_n in enumerate(rx_tx_list):
        plt.semilogy(snr_db_range, ser_array[:, mrc_idx], styles[mrc_idx], label=f'BMF (N={mrc_n})')
...
def stc(s_int, s_symbols, tx):
    ...
    for snr_db_idx, snr_db in enumerate(snr_db_range):
        # Convert SNR from dB to linear and calculate required noise power
        snr_linear = 10 ** (snr_db / 10.0)
        rho = 1. / np.sqrt(snr_linear)  # Noise standard deviation
        ...
        # Count the errors and calculate SER
        ser_stc_array[snr_db_idx] = np.sum(s_stc_int != s_stc_detected) / (
            num_symbols)  # TODO: * 2 symbols per STC block ?
